...
static void i2c_core_process(struct xiic_i2c *i2c)
{
 ...
 */
static int i2c_core_wait(struct xiic_i2c *i2c,
...
static int ocores_poll_wait(struct xiic_i2c *i2c)
{
	...
	if (1 == i2c->nmsgs && 0 == i2c->msg->len && i2c->state == STATE_START && !(i2c->msg->flags & I2C_M_RD)) { 	/* for i2cdetect I2C_SMBUS_QUICK mode*/
		err = i2c_core_wait(i2c, XIIC_IISR_OFFSET, mask, mask, msecs_to_jiffies(30));
		mutex_lock(&i2c->lock);
		...
		if (mask & XIIC_SR_TX_FIFO_EMPTY_MASK){
			err = i2c_core_wait(i2c, XIIC_SR_REG_OFFSET, mask, XIIC_SR_TX_FIFO_EMPTY_MASK, msecs_to_jiffies(30));
			mask &= ~XIIC_SR_TX_FIFO_EMPTY_MASK;
		}
		if (0 == err){
			err = i2c_core_wait(i2c, XIIC_SR_REG_OFFSET, mask, 0, msecs_to_jiffies(30));
		}
		mutex_lock(&i2c->lock);
...
static void i2c_core_process_polling(struct xiic_i2c *i2c)
{
	while (1) {
		int err;
		...
		i2c_core_process(i2c);
	}
}

static int xfer_core(struct xiic_i2c *i2c,
...
MODULE_ALIAS("platform:fpga-xiic-i2c");
